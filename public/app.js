...
function onPosition(pos) {
  ...
  if (speed != null && !Number.isNaN(speed)) {
    lastSpeedKn = speed * MS_TO_KN;
  }
  if (heading != null && !Number.isNaN(heading)) {
    lastCourseDeg = heading;
  }
  ...
  if (lastFix) {
    ...
    if (dist > 5) {
      if (lastCourseDeg == null || heading == null) {
        lastCourseDeg = bearingDeg(lastFix.lat, lastFix.lon, latitude, longitude);
      }
      if ((speed == null || Number.isNaN(speed)) && dt > 0) {
        lastSpeedKn = (dist / dt) * MS_TO_KN;
      }
    }
  }
